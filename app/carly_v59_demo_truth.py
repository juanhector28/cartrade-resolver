"""Carly v59 demo-truth capsule: unknown-make honesty + authoritative price ranges."""
...
def _known_makes_normed() -> set[str]:
    out = set()
    for alias in getattr(v31, "_BRAND_ALIASES", {}) or {}:
        out.add(v58.v28._norm(alias))
    for alias in getattr(v58, "_BRANDS", {}) or {}:
        out.add(v58.v28._norm(alias))
    return out


def _unknown_make_token(text: str) -> str | None:
    if not text or not v52._SEARCH_RE.search(text):
        return None
    if v58._brand_in_turn(text) or v31._canonical_exact(text):
        return None
    known = _known_makes_normed()
    for raw in re.findall(r"[A-Za-zÁÉÍÓÚÜÑáéíóúüñ][\w-]*", text):
        token = v58.v28._norm(raw)
        if not token or token in _STOPWORDS or token in known:
            continue
        if _YEARISH.match(raw) or _NUMBERISH.match(raw):
            continue
        if raw[:1].isupper() and len(raw) >= 3:
            return raw
    return None
...
def _no_inventory_reply(token: str, country: str) -> dict[str, Any]:
    ...
    return {
        "phase":"conversation",
        "reply": f"No tengo {token} en el inventario certificado de {where} en este momento. No te voy a mostrar sustitutos como si lo fueran. Si querés, decime tu presupuesto y uso principal y te muestro alternativas reales, marcadas explícitamente como alternativas.",
        "recommendations":[], "explore":[], "token_path":"deterministic_no_inventory_v59",
        "route_precedence":"unknown_make_v59", "llm_calls":0,
    }
...
def _constraints_with_floor(body: Any, *args: Any, **kwargs: Any) -> dict[str, Any]:
    ...
    try:
        ...
        if rng:
            c["price_min"], c["total_budget"] = rng
    except Exception:
        log.exception("Carly v59 price range constraint enrichment failed")
    return c
...
def _filter_price_range(rows: list[dict], c: dict[str, Any]) -> list[dict]:
    floor = c.get("price_min")
    ceiling = c.get("total_budget") if floor is not None else None
    if floor is None:
        return rows
    floor = float(floor)
    ...
    for row in rows or []:
        try:
            ...
            continue
        if price < floor or (ceiling is not None and price > ceiling):
            continue
        kept.append(row)
    return kept
...
def _rows_with_range_v31(c: dict[str, Any], *args: Any, **kwargs: Any) -> list[dict]:
    ...
    try:
        return _filter_price_range(rows, c)
    except Exception:
        log.exception("Carly v59 v31 range filter failed closed")
        return [] if c.get("price_min") is not None else rows
...
if _prior_v46_query_rows is not None:
    def _rows_with_range_v46(c: dict[str, Any], *args: Any, **kwargs: Any) -> list[dict]:
        ...
        try:
            ...
            log.exception("Carly v59 v46 range filter failed closed")
            return [] if c.get("price_min") is not None else rows
...
def install(app: Any) -> None:
    for route in getattr(app, "routes", []):
        ...
        if prior is None or dependant is None or getattr(prior, "_carly_v59_demo_truth", False):
            continue
        @wraps(prior)
        def endpoint(*args: Any, __prior=prior, **kwargs: Any):
            body = v58._request_body(args, kwargs)
            try:
                users = [m for m in v52._messages(body) if v52._role(m) == "user"]
                if users:
                    latest = v58._clean_user(v52._content(users[-1])).strip()
                    token = _unknown_make_token(latest)
                    if token:
                        return _no_inventory_reply(token, v52._country(body))
            except Exception:
                log.exception("Carly v59 unknown-make guard failed; falling through")
            return __prior(*args, **kwargs)
        endpoint._carly_v59_demo_truth = True
        endpoint._carly_v59_prior = prior
        route.endpoint = endpoint
        dependant.call = endpoint
        break
    log.warning("CARLY_V59 installed unknown_make_guard=true price_floor=true vision_inline_max=%s vision_jit_flag=%s", os.getenv("CARLY_VISION_INLINE_MAX", "(default 3)"), os.getenv("CARLY_VISION_JIT_ENABLED", "(default 1 = ON)"))


_probe_unknown = _unknown_make_token("Busco un Bugatti Chiron 2025")
if not _probe_unknown or "bugatti" not in _probe_unknown.lower():
    raise RuntimeError("Carly v59 unknown-make guard regression")
if _unknown_make_token("Busco un Toyota SUV") is not None:
    raise RuntimeError("Carly v59 Toyota false positive")
_probe_rng = v52._price_range("SUV confiable entre USD 15,000 y 25,000")
if _probe_rng != (15000.0, 25000.0):
    raise RuntimeError(f"Carly v59 range parse regression: {_probe_rng!r}")
