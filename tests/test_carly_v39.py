from app import main_v39 as v39
...
def test_prompt1_forces_retrieval_and_keeps_negative_constraints():
    ...
    assert v39._should_retrieve(c) is True

    assert v39._hard_ok(_card("Nissan", "Frontier", body="pickup"), c) is False
    assert v39._hard_ok(_card("Ford", "Ranger", body="pickup"), c) is False
    assert v39._hard_ok(_card("Honda", "HR-V"), c) is True
...
def test_crv_is_strong_exact_and_fallback_is_brand_limited():
    ...
    assert v39._hard_ok(_card("Mazda", "CX-30"), alt) is True
    assert v39._hard_ok(_card("Kia", "Forte", body="sedan"), alt) is False
    assert v39._hard_ok(_card("Nissan", "Rogue"), alt) is False
...
def test_soft_corolla_civic_equivalent_is_not_exact():
    ...
    corolla = _card("Toyota", "Corolla", body="sedan")
    venue = _card("Hyundai", "Venue", body="suv")
    assert v39._score(corolla, c) > v39._score(venue, c)


def test_six_passenger_nuclear_prompt_can_accept_real_three_row_vehicle():
    c = v39._constraints(_body(
    ...
    assert c["total_budget"] == 18000
    assert c["monthly_max"] == 430
    assert "pickup" in c["avoid_body"]

    outlander = _card(
        "Mitsubishi", "Outlander", year=2022, price=15950, monthly=379,
        description="Mitsubishi Outlander ES 2022. 3 filas de asientos.",
    )
    explorer = _card(
        "Ford", "Explorer", year=2022, price=16500, monthly=393,
        description="Ford Explorer XLT. 3 filas de asientos.",
    )
    sport = _card(
        "Mitsubishi", "Outlander Sport", year=2025, price=13950, monthly=332,
    )
    assert v39._mission_ok(outlander, c) is True
    assert v39._mission_ok(explorer, c) is True
    assert v39._mission_ok(sport, c) is False


def test_luggage_for_four_prefers_suv_over_same_base_score_sedan(monkeypatch):
    c = v39._constraints(_body(
        "Somos 4, llevamos bastante equipaje. Automático, máximo US$425 al mes."
    ))
    monkeypatch.setattr(v39, "_ORIG_SCORE", lambda card, constraints: 50.0)
    suv = _card("Honda", "HR-V", body="suv")
    sedan = _card("Toyota", "Corolla", body="sedan")
    assert v39._score(suv, c) > v39._score(sedan, c)
