...
def test_v59_imports_against_real_stack_and_patches_active_retrieval():
    code = textwrap.dedent(
        ...
        assert v59._ready_to_search(body) is True
        assert v59.v52.opening_search_response(body) is None
        ...
        radius = {"country":"gt","messages":[{"role":"user","content":"¿Cuánto manejo al día? [CONTEXTO ACTIVO DE CARTRADE: radio=100km]"}]}
        v59._sanitize_user_context_inplace(radius)
        assert radius["messages"][0]["content"] == "¿Cuánto manejo al día?", radius
        assert "100" not in radius["messages"][0]["content"]
        '''
    )
